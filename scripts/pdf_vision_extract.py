...
━━━ headings 規則 ━━━【嚴格標準：通常每頁 heading 不超過 3 個】
只列出字體在視覺上**明顯大於正文**的章節標題：
- level 2：整頁中字體最大的大節標題（如「3.1 自然語言處理」「4. 分散式資料處理框架」）
  ⚠️ 注意：頁首反覆出現的章節標題（如「第三章 機率統計基礎」）是頁眉裝飾，不列入 headings
- level 3：字體明顯大於正文的子節，前提是字體確實較正文大
- level 4：字體略大於正文但確實可辨為獨立小節標題（非常罕見）

⚠️ 以下情況**一律不列入 headings**：
  - 頁首/頁眉的章節名稱（每頁固定出現在頂端的裝飾性標題）
  - 粗體但字體大小與正文相同的項目（即使有編號如「1.」「（1）」「A.」）
  - 條列清單的標籤項（如「A. 優點：」「B. 缺點：」「◆ 注意：」）
  - 一頁中出現 4 個以上格式相同的編號項目 → 全部視為段落/清單，不是標題

━━━ markdown 規則 ━━━
1. 省略：頁碼（如「3-24」）、頁首章節名稱（如「第三章 人工智慧基礎概論」）、頁尾資訊
2. 標題：以 ## / ### / #### 對應 heading level，與 headings 陣列一致；粗體但非標題的項目用 **粗體** 格式
