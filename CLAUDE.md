...
```bash
# Step 1: PDF 每頁轉圖片送 Claude Vision，結果快取於 data/初級/pages_cache/
uv run python3 scripts/pdf_vision_extract.py --all        # 兩科全跑（約 133 頁，~$1.6）
...
`pdf_vision_extract.py`, `generate_questions.py`, and `audit_chapters.py` require `ANTHROPIC_API_KEY` environment variable.
...
- **`scripts/guide_to_md.py`**: Deterministic span-based guide extractor — no LLM required. Uses PyMuPDF font size/bold flags to classify 6 heading levels (L2 ≥18pt bold → `##`, L3 ≥13pt bold → `###`, L4 bold `（N）` → `####`, L5 bold `A.` → `#####`, L6 bullet → `-`, L0 body). Merges continuation lines, builds a nested tree, serialises to Markdown + JSON. Validates key term retention (default threshold 95%). Outputs `subject{N}_guide.json`, `subject{N}_guide.md`, `subject{N}_guide_nested.json`, `subject{N}_validation_report.json` to `data/初級/guide/`. Supports `--subject`, `--all`, `--chapter`, `--threshold`.
- **`scripts/pdf_vision_extract.py`**: Renders each PDF page to PNG (2× scale via PyMuPDF) and calls Claude Vision API to extract structured Markdown. Results are cached per page at `data/初級/pages_cache/{key}/page_NNN.json` (`type`: content/practice/skip, `markdown`, `usage`). Re-runs only process missing/failed pages. Supports `--subject`, `--all`, `--dry-run`, `--force`, `--page`.
