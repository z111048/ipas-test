<!-- 2026-08-30（使用者授權）：將 pipeline 腳本目錄與 publication overlay 詳節路由至 companion，並同步考題 14 卷／715 題。備份：playbook/backups/CLAUDE.md.bak-2026-08-30-3。 -->
...
3. **破壞性腳本**：`export_guide_outline_data.py` 會 staging content／outlines／exact source assets，
   ...
   失敗 rollback，單等級保留其他等級。常規完整重跑必帶 `--all-levels --use-guide-tree`
   （見 `playbook/deterministic-publication-overlays.md`）。
...
- **產圖依賴另一個 repo 的容器**：`~/projects/codex-imggen`（`docker compose up -d`，
  ...
  （產物檔名慣例隨 codex-cli 版本變，會靜默壞掉）。詳見 `playbook/pipeline-script-catalog.md`。
