...
- **概念圖卡 page** (`frontend/src/pages/VisualCardsPage.tsx`, route `/visuals`): card-style browser for all generated guide infographics from `guideImages.json`, grouped by chapter with level/subject/chapter filters, keyword search, and incremental "載入更多章節" paging (少量多餐). The 727-image `guideImages.json` is route-lazy so it does not bloat the homepage bundle. Linked from the sidebar top nav and homepage hero.
