...
| 要做的事 | 讀這份 |
|---|---|
| 跑任何 pipeline、查腳本用途、輸出檔案、驗證清單、**s1c4 手動修正** | `playbook/pipeline-reference.md`（有目錄，用 Grep 跳小節） |
...
| 學習指引 OCR 校正的現況與待接工作 | `playbook/06-guide-ocr-recalibration.md`（任務已完成；規則已進 pipeline-reference） |
| **小節粒度出題（進行中）**：codex CLI 出題、全量前的驗證缺口 | `playbook/07-question-generation.md` |
...
2. **商品化文案**：前端使用者可見文案不得透露教材/題目由 AI 生成。內部變數/路由 key
   （`codex100`、`summary.ai` 等）可保留。既定對外命名：「精選 100 題」（≠Codex 100 題）、
   「章節練習/章節模擬練習題」（≠AI 模擬）、「概念圖卡」（=AI 資訊圖，頁面 `/visuals`）。
   「AI 應用規劃師」（考試名）與「生成式 AI」（考綱主題）屬正當用語。
3. **破壞性腳本**：`export_guide_outline_data.py` 必帶 `--all-levels`（否則中級檔案被 rmtree
   永久刪除），跑完必補 s1c4 兩個手動修正（腳本在 `pipeline-reference.md` §10）。
...
資料 pipeline 腳本都支援 `--level`，但**預設值不一致**：多數預設 `初級`，中級專屬 pipeline
...
`llm_review_guide_headings`）預設 `中級`——**執行任何 pipeline 腳本一律顯式帶 `--level`**。
...
- 多數腳本硬編 `BASE = Path('/home/james/projects/ipas-test')`（少數用 `Path(__file__)` 相對解析），搬 repo 要改。
