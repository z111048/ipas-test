...
`pages_cache/` 現在由 §1a 的 `ocr_extract.py` 產生。以下只在需要重建 Vision 版
（OCR 產物不可用、或要 A/B 比較）時才跑；**會覆蓋 OCR 版快取，跑之前先備份**。
...
`page_extract/`——那支腳本**只在備份不存在時才備份**，直接重跑會疊在已合併的結果上。
...
⚠️ **捲一次不夠**。公式（KaTeX）與圖片是載入後才撐開高度的，早算的位置會失準——
實測目標會被推到畫面外一萬多 px，只重試到 2.5 秒仍差 3,463 px。現行做法是每 250 ms
檢查 `scrollHeight`，變動就重捲，連續 4 次不變或超過 12 秒才停，使用者一動（wheel／
touch／鍵盤）就停手。改這段前先看實測數字，不要退回單次捲動。
