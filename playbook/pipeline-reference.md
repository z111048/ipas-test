...
現況：28 筆中 **26 筆已套用**（自動 16、人工 21 條目涵蓋 10 筆），剩 2 筆：
- 初級 3-27（假說檢定名詞表）：**勘誤表自己的巢狀表格 OCR 結構是壞的**，
  「更正後」欄只抓到標題、表身沒抓到，無從得知官方要改成什麼。要看原始 PDF。
  （順帶一提，PDF 文字層證實原書印的是「拒絕 H0 → Type II 錯誤（α）」，實為 Type I 之誤。）
- 中級 5-21（Recall 分母 TP+FP → TP+FN）：`ocr_formulas` 對同頁相同 LaTeX 做了去重，
  而 Precision 的 `TP/(TP+FP)` 與 Recall 誤印的 `TP/(TP+FP)` 文字完全相同被併成一筆，
