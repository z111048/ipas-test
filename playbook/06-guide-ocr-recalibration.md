...
## §12 題庫引文對齊驗收（2026-08-07，接在 §11 之後）

**結論：新 OCR 沒有打壞題庫。** 反而暴露並修掉了一批既有的抽取污染。

### 新工具：`scripts/verify_question_guide_alignment.py`

唯讀量測，`--level` 可選，`--json` 出完整報告。量三件事：

1. 章節引用完整性（題庫的 `chapter_id` / `chapter_title` vs `toc_manifest` 與現行 guide）。
2. `guide_exercises` 逐字引文（題幹＋選項對現行 page_clean 文字，含跨頁串接），
   並與 `page_extract_before_ocr_merge/` 舊版比，區分「本輪造成的退化」與「舊版就這樣」。
3. 章節正文新舊漂移（給精選 100 題那種「引章節但無逐字引文」的題目當風險指標）。

兩個踩過的坑，改腳本時別重犯：

- **題幹與選項要分開加權比對**。頁面上選項被「（A）」隔開，串成一條字串用最長共同
  子串比會被嚴重低估（首跑 64/69 誤報成「找不到」）。
- **題目常跨頁**（選項 C/D 印在下一頁）。單頁比對會低估；而補比「本頁＋下一頁」後，
  offset 迴圈要依 `|offset|` 由小到大，否則前一頁也拿滿分，整批被誤標成「頁碼偏移」。
- 全書掃描是主要成本（跑一次兩級約 20–40 分鐘）。引用頁已命中就別再掃全書。

### 量出來的規模

| | 初級 | 中級 |
|---|---|---|
| 章節引用檢查 / 失效 | 226 / 0 | 512 / 0 |
| `guide_exercises` 題數 | 69 | 110 |
| 修正前：引用頁逐字命中 | 54 | 89 |
| 修正前：本輪 OCR 造成的退化 | **1** | 0 |
| 修正後：引用頁逐字命中 | **69（全數）** | **110（全數）** |

章節正文漂移：初級 7 章平均相似度 0.944、中級 34 章 0.915，**無一章 <0.5**。
精選 100 題 300 題引用章節全部存在，tag 命中率 90.4%（比舊版少的只有 1 題）。
中級「練習題混入章節正文」90/110 → 0/110（這是 §8 想要的效果，現在有數字）。

### 修了什麼（commit `ec62314`）

`export_guide_embedded_exercises.py` 有兩個尾端污染，兩者都與本輪 OCR 無關，是既有債：

- **頁眉黏進選項**：題塊跨頁時吃進下一頁頁眉，例如選項 D 變成
  「迭代次數第三章 人工智慧基礎概論」（初級 14 題、中級 17 題）。
  修法：頁眉靠出現頻率認（重複出現在多頁首行），不寫死章名；只剝該頁首行。
- **解析吃到下一章正文**：最後一題的解析沒有結束標記，一路吃到下一頁的新章內文，
  最誇張一筆長 18,485 字。修法：依 `clean_pdf_page_text` 的 `continues_to_next`
  在頁尾收束。

⚠️ **題目區塊只在四個選項齊全時才於頁尾收束**。無條件收束會讓「選項真的跨頁」的題
整題消失——中級 `mid-s3c6gq010` 就是這樣掉的，重跑時題數會從 40 變 39。

順帶讓兩筆官方勘誤生效（勘誤已在 page_clean 層，重跑抽取器就會吃到）：

- `s1c2gq003`：**答案 B→A**（官方勘誤改的是正確答案，不只文字），並補回正確解析
  （原本是「解析：用於估計結果不穩定時。」這種串行污染）。
- `s1c2gq007`：題幹與選項 (B)(D) 換成勘誤後文字（答案字母仍是 B）。

中級 110 題完全沒被勘誤打到。

### 重跑順序補充（接 §11 那份）

`export_guide_embedded_exercises.py --level all` 要排在 `apply_errata.py` 與
`clean_pdf_page_text.py` **之後**——它讀的是 `page_clean` 的 `cleaned_text`，
勘誤與清洗都在那一層，順序反了就吃不到勘誤。

### 驗證狀態

`verify_data_alignment` 兩級通過、`npm run build` 零 TS 錯誤（8m35s）、
引文複驗 179 題全數在引用頁命中、0 退化。
