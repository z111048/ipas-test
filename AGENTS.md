...
> ⚠️ 本檔內容較舊，僅供 Codex CLI 讀取。權威版本：`CLAUDE.md`（不變量與路由）＋
> `playbook/pipeline-reference.md`（pipeline 細節）。兩者衝突時以 playbook 為準。
> 更新腳本行為時改 playbook，不要改這裡。
...
- `python3 scripts/export_guide_outline_data.py --all-levels --use-guide-tree`（⚠️ 必帶 `--all-levels`，否則中級 guideContent 會被 rmtree 永久刪除；跑完補回 s1c4 修正，見 playbook/pipeline-reference.md §10）: refresh frontend guide outline metadata and split guide content imports from validated guide tree artifacts. Omit `--use-guide-tree` only when intentionally using the legacy inline hierarchy path.
